...
pub(crate) fn run_list(cmd: &ListCmd) {
...
pub(crate) fn run_extract(cmd: ExtractCmd) {
    let data = load_file(&cmd.reader.source);
    ...
    let base_path = match cmd.out_dir {
        Some(dir) => dir,
        None => PathBuf::new(),
    };
    for files in files_to_extract.into_values() {
        for (idx, file) in files.iter().enumerate() {
            ...
            cmd.case.convert(&mut out_path);
            if files.len() > 1 {
                out_path.add_extension(format!("{idx}"));
            }
            let out_path = base_path.join(out_path);
            println!("Extracting {} to {}", file.path().unwrap(), out_path.display());
