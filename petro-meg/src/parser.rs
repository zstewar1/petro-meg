...
pub fn parse_v1(bytes: &[u8]) -> MegFileContentsV1<'_, 'static> {
...
#[instrument(skip(bytes))]
pub fn parse_v1_opt<'b, 'o>(
    bytes: &'b [u8],
    options: &'o ParseOptions,
) -> MegFileContentsV1<'b, 'o> {
    MegFileContentsV1::new(bytes, options)
}
...
impl<'b, 'o> Iterator for MegFileContentsV1<'b, 'o> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if self.file_records.is_empty() {
            ...
            match record_to_file(self.options, self.bytes, names, expected_index, &record) {
...
fn get_names_or_start_parse<'b, 'n>(
    ...
    Ok(match names {
        Some(names) => names,
        None => {
            let (n, r) = parse_header_and_names_v1(bytes, options)?;
            *file_records = r;
            names.insert(n)
        }
    })
...
/// table.
fn parse_header_and_names_v1<'b>(
    full_file: &'b [u8],
    options: &ParseOptions,
