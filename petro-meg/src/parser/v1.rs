...
impl<'b, 'o> Iterator for MegFileContentsV1<'b, 'o> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if self.file_records.is_empty() {
            ...
            match record_v1v2_to_file(self.options, self.bytes, names, expected_index, &record) {
...
fn parse_header_and_names<'b>(
    ...
    let (header, mut cursor) = HeaderV1::split_off(full_file).ok_or(
        MegParseError::NotEnoughBytesForHeader {
